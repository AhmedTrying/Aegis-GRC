...
serve(async (req) => {
  ...
  try {
    ...
    const { error: updErr } = await admin.auth.admin.updateUserById(id, { ban_duration: "100y" } as any);
    if (updErr) return new Response(JSON.stringify({ error: updErr.message }), { status: 400, headers: cors(req) });

    await admin.from("audit_logs").insert({ org_id: prof.org_id, actor: user.id, actor_email: user.email, entity_type: "users", entity_id: id, action: "deactivate_user" });
