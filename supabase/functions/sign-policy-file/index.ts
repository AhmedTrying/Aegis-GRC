...
serve(async (req) => {
  ...
  try {
    ...
    if (!storage_path || typeof storage_path !== "string") {
      return new Response(JSON.stringify({ error: "storage_path required" }), { status: 400, headers: getCorsHeaders(req) });
    }

    // Verify org ownership via prefix
    ...
    const { data: signed } = await admin.storage.from("policies").createSignedUrl(storage_path, 60 * 60);
    return new Response(JSON.stringify({ signed_url: signed?.signedUrl || null }), { headers: getCorsHeaders(req) });
  } catch (e) {
